...
#fasttext.util.download_model("en", if_exists="ignore")# bu model i嚙箠n bir kere indirdikten sonra kapatilacak
...
def userLogin(request):
    ...
    #makaleKayit()    #bu k覺s覺m tum makaleler yuklenikten sonra yoruma alinmali
    if request.method == 'POST':
        email = request.POST.get('email')
        password = request.POST.get('password')
...
def updateInterestAreas(request):
    if 'user_email' in request.session:
        ...
        if user_data:
            ...
            # Kullanici verileri bulunamadiysa hata mesaji goster
            messages.error(request, 'Kullan覺c覺 verileri bulunamad覺.')
            return redirect('home')
    else:
        # Kullanici oturumu kapali ise ana sayfaya yonlendir
        return redirect('home')
        ...
        if ilgi_alanlari:
            ...
            try:
                ...
                if user_email:  # Kullanici oturumu var mi kontrol et
                    ...
                    messages.error(request, 'Kullanici oturumu bulunamad覺.')
            except Exception as e:
            ...
            messages.error(request, 'Lutfen en az bir ilgi alan覺 se癟in.')
